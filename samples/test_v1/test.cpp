...
void flatbuffers_serialization_test(int iterations){


    flatbuffers::FlatBufferBuilder builder;

    long ids_data[] = { 34492,6603,44033,8874,47607,38416,20395,29192,38620,36775,
...
//    int a = sizeof(ids_data);
    auto strings_data="shgfkghsdfjhgsfjhfgjhfgjsffghgsfdhgsfdfkdjhfioukjhkfdljgdfkgvjafdhasgdfwurtjkghfsdjkfg";
    auto ids = builder.CreateVector(ids_data, 1000);
    auto strings = builder.CreateString(strings_data);


    auto start = std::chrono::high_resolution_clock::now();
    for (int i = 0; i < iterations; ++i) {
        auto mloc = CreateRecord(builder,ids,strings);
        builder.Finish(mloc);
        auto record = GetRecord(builder.GetBufferPointer());
//        record->ids();
//        std::cout << "Test.................." << record->ids()->Get(2) << std::endl;
    }
    auto finish = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(finish - start).count();
    std::cout << "Time = " << duration << " milliseconds ==== iterations: " <<  iterations << std::endl;

}

void flatbuffers_serialization_test2(int iterations) {
    // load FlatBuffer schema (.fbs) and JSON from disk
    std::string schemafile;
    std::string jsonfile;
    bool ok = flatbuffers::LoadFile("samples/test_v1/test.fbs", false, &schemafile) &&
              flatbuffers::LoadFile("samples/test_v1/test.json", false, &jsonfile);
    if (!ok) {
...
void flatbuffers_serialization_test3(int iterations) {

}
int main(int /*argc*/, const char * /*argv*/[]) {
    flatbuffers_serialization_test(1000000);
    flatbuffers_serialization_test2(1000);
}